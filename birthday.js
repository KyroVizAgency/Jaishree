...
const gsap = window.gsap || globalThis.gsap;
...
function buildFilm(m){
  ...
  t.fromTo(nockProxy, { val: m.drawnNock }, { val: REST_NOCK, duration: 0.5, ease: 'elastic.out(1,0.34)', onUpdate: applyNock }, 0)
   ...
   .to([eyebrow, heroName, hint], { opacity: 0, duration: 0.2, ease: 'power1.out' }, 0);
...
function enter(){
  ...
  gsap.set(heroName, { opacity: 0 });                    // opacity only so transform: translate(var(--name-x), var(--name-y)) is preserved
  ...
  tl.to(target,   { opacity: 1, y: 0, scaleX: 1, scaleY: 1, duration: 0.8, ease: 'power3.out' }, 0.1)
    ...
    .to(heroName, { opacity: 1, duration: 0.7, ease: 'power3.out' }, 0.4)
